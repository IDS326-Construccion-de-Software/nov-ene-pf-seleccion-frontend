...
import { AppLayout3Config } from '.';

// Interface defining the properties of the layout provider context
export interface IAppLayout3ProviderProps {
...
const initalLayoutProps: IAppLayout3ProviderProps = {
  layout: AppLayout3Config, // Default layout configuration
...
const AppLayout3Context = createContext<IAppLayout3ProviderProps>(initalLayoutProps);

// Custom hook to access the layout context in other components
const useAppLayout3 = () => useContext(AppLayout3Context);

// Provider component that sets up the layout state and context for Demo3 layout
const AppLayout3Provider = ({ children }: PropsWithChildren) => {
  ...
  const layoutConfig = deepMerge(AppLayout3Config, getLayout(AppLayout3Config.name));
...
export { AppLayout3Provider, useAppLayout3 };
