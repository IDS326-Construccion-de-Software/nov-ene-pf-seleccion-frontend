...
import { appLayoutConfig } from './';

// Interface defining the structure for layout provider properties
export interface IAppLayoutProviderProps {
...
const initalLayoutProps: IAppLayoutProviderProps = {
  layout: appLayoutConfig, // Default layout configuration
...
const AppLayoutContext = createContext<IAppLayoutProviderProps>(initalLayoutProps);

// Custom hook to access the layout context
const useAppLayout = () => useContext(AppLayoutContext);

// Layout provider component that wraps the application
const AppLayoutProvider = ({ children }: PropsWithChildren) => {
  ...
  const getLayoutConfig = () => {
    return deepMerge(appLayoutConfig, getLayout(appLayoutConfig.name));
  };
  ...
  const setSidebarCollapse = (collapse: boolean) => {
    ...
    updateLayout(appLayoutConfig.name, updatedLayout); // Updates the layout with the collapsed state
