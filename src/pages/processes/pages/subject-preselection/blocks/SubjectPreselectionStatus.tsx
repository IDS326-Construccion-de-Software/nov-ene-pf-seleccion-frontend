...
import { ResumenCarga } from '@/interfaces/preselection/preselection.interfaces';
...
const SubjectPreselectionStatus = ({ resumen, canModify = true }: StatusProps) => {
  const [periodoActivo, setPeriodoActivo] = useState<PeriodoConfigResponse | null>(null);
  const [enPeriodoPreseleccion, setEnPeriodoPreseleccion] = useState(false);
  ...
  useEffect(() => {
    const fetchPeriodo = async () => {
      try {
        ...
        // Verificar si estamos en el período de preselección
        const ahora = new Date();
        const inicio = new Date(data.preseleccionInicio);
        const fin = new Date(data.preseleccionFin);
        setEnPeriodoPreseleccion(ahora >= inicio && ahora <= fin);
      } catch (error) {
    ...
    <Card className="bg-white/50 border-gray-200">
      <CardContent className="p-4">
        {!canModify && (
          <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-3">
            ...
            <p className="text-sm text-amber-900">
              El periodo de modificación de preselección ha finalizado o no está activo.
            </p>
          </div>
        )}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        <div className='flex gap-4'>
                          <div className="flex flex-col text-xs text-gray-600 font-medium">
                            ...
                            <span className="text-nowrap">
                              {formatearFecha(periodoActivo.preseleccionInicio)}{' '}
                              {formatearHora(periodoActivo.preseleccionInicio)}
                            </span>
                          </div>
                          <div className="flex flex-col text-xs text-gray-600 font-medium">
                            <span className="font-bold uppercase text-gray-900 ">Fecha hasta</span>
                            <span className="text-nowrap">
                              {formatearFecha(periodoActivo.preseleccionFin)}{' '}
                              {formatearHora(periodoActivo.preseleccionFin)}
                            </span>
                          </div>
                        </div>
                      </div>
          ...
          {/* <div className="flex items-center gap-3 bg-blue-50/50 p-3 rounded-xl border border-blue-100 lg:w-auto w-full">
            <div className="size-8 rounded-full bg-blue-100 flex items-center justify-center shrink-0">
              <KeenIcon icon="information" className="text-blue-600" />
            </div>
            <p className="text-[11px] text-blue-700 leading-tight">
              El costo es estimado y puede variar según los cargos adicionales.
            </p>
          </div> */}
        </div>
      </CardContent>
...
export { SubjectPreselectionStatus };
