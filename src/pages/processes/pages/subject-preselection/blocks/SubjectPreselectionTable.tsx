...
interface SubjectPreselectionTableProps {
...
const SubjectPreselectionTable = ({
  subjects,
  canModify = true,
  currentPage,
  itemsPerPage,
      ...
      }),
    {
      onSuccess: (data) => {
        if (data.success) {
          ...
          queryClient.invalidateQueries(['oferta-preseleccion']);
          queryClient.invalidateQueries(['resumen-preseleccion']);
        } else {
      ...
      onError: (error: any) => {
        const errorMsg =
          error?.response?.data?.message || 'Ocurrió un error al intentar guardar la preselección';
  ...
  const cancelMutation = useMutation(
    (preseleccionId: number) => cancelPreselection(preseleccionId, usuarioId),
    {
      ...
      onError: (error: any) => {
        const errorMsg =
          error?.response?.data?.message || 'Ocurrió un error al intentar quitar la preselección';
        toast.error(errorMsg);
      }
    }
  );
  ...
  const renderActionButton = (
    ...
    asignaturaPuedePreseleccionar: boolean,
    ...
    if (!asignaturaPuedePreseleccionar || !seccion.estatusValidacion.puedeInscribir) {
      return (
        <Button
          size="icon"
          variant="destructive"
                    ...
                    >
                      ...
                      <TableCell className="font-bold text-gray-900">
                        <div className="flex flex-col min-h-[10px] justify-center">
                          ...
                          {/* Mostrar el motivo si zno puede preseleccionar la asignatura */}
                          {!subject.puedePreseleccionar && subject.motivoBloqueo && (
                      ...
                      <TableCell className="text-center text-nowrap">
                        {selectedSection ? (
                          ...
                          <Badge
                            variant="outline"
                            className="bg-gray-100 text-gray-800 border-gray-300 rounded px-2 font-bold"
                          >
                            No disponible
                          </Badge>
                        ) : (
                          ...
                          <div className="flex items-center gap-2">
                            <div
                              className={`size-1.5 rounded-full shrink-0 ${
                                !hasConflict && subject.puedePreseleccionar
                                  ? 'bg-green-500'
                                  : 'bg-red-500'
                              }`}
                            ></div>
                        ...
                        <TableRow key={section.seccionId} className="hover:bg-gray-50/50">
                          ...
                          <TableCell className="text-center">
                            {section.seleccionada ? (
                              ...
                              <Badge
                                variant="outline"
                                className="bg-gray-100 text-gray-700 border-gray-300 rounded px-1.5 font-medium text-nowrap"
                              >
                                Bloqueada
                              </Badge>
                          ...
                          <TableCell className="text-right">
                            {renderActionButton(
                              section,
                              subject.puedePreseleccionar,
                              subject.asignatura,
                              subject.preseleccionId
                            )}
                          </TableCell>
                        </TableRow>
                      ))}
...
export { SubjectPreselectionTable };
